                ...
                if (source === ctx.MESSAGE_SOURCES.CONTENT_SCRIPT && target === ctx.MESSAGE_SOURCES.BACKGROUND) {
                    ...
                    if (isManaged && ctx.initPopupMessage && ctx.initPopupMessage.has(sender.tab.id)) {
                        ...
                        chrome.tabs.sendMessage(sender.tab.id, pendingMessage.message)
                            ...
                            .catch(error => ctx.debugLogger.error(ctx.DebugLogType.BACKGROUND, `[BACKGROUND] Error sending (pending) SHOW_PROVIDER_VERIFICATION_POPUP to tab ${sender.tab.id} (promise catch):`, error));
                    }
                    ...
                    if (isManaged && ctx.providerDataMessage && ctx.providerDataMessage.has(sender.tab.id)) {
                        ...
                        chrome.tabs.sendMessage(sender.tab.id, pendingMessage.message)
                            .then(() => {
                                if (chrome.runtime.lastError) {
                                    ctx.debugLogger.error(ctx.DebugLogType.BACKGROUND, `[BACKGROUND] Error sending (pending) PROVIDER_DATA_READY to tab ${sender.tab.id}:`, chrome.runtime.lastError.message);
                                }
                            })
